...
There are still a few considerations related to how engines work that are worth mentioning:
  ...
  * Some tips I suggest:
    ...
    * It might be useful to create a few scripts to run migrations and test suite or install gems across all the engines. The scripts will iterate over the engines folder and run the command inside each one.
