...
- Rendering component `param-one`
- Rendering component `param-two-content`
...
To run the tests we just simply need to write `npm test`. This command will run all the tests we have under the `__tests__` folder. If we want to just run one particular test, we can do `npm test __tests__/screens/one_particular_test.js` instead.
